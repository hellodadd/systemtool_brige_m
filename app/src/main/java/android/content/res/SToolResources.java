...
public class SToolResources extends SToolResourcesSuperClass {
	...
	private static final ThreadLocal<LinkedList<MethodHkParam>> sIncludedLayouts = new ThreadLocal<LinkedList<MethodHkParam>>() {
		@Override
		protected LinkedList<MethodHkParam> initialValue() {
	...
	private static void setReplacement(int id, Object replacement, SToolResources res) {
	...
	public XmlResourceParser getAnimation(int id) throws NotFoundException {
		...
		if (replacement instanceof SToolResForwarder) {
			Resources repRes = ((SToolResForwarder) replacement).getResources();
			int repId = ((SToolResForwarder) replacement).getId();
					...
					synchronized (sResourceNames) {
						...
						if (resNamesInner != null) {
							synchronized (resNamesInner) {
								...
								MethodHkParam top = sIncludedLayouts.get().peek();
								if (top != null)
	...
	private static native void rewriteXmlReferencesNative(long parserPtr, SToolResources origRes, Resources repRes);
	 ...
	 */
	private static int translateResId(int id, SToolResources origRes, Resources repRes) {
		try {
			...
			if (repResDefined && !entryType.equals("id"))
				origRes.setReplacement(origResId, new SToolResForwarder(repRes, id));
	...
	public int addResource(Resources res, int id) {
		...
		synchronized (sReplacements) {
			if (sReplacements.indexOfKey(fakeId) < 0)
				setReplacement(fakeId, new SToolResForwarder(res, id));
		}
		return fakeId;
	}
	 ...
	 */
	private static int translateAttrId(String attrName, SToolResources origRes) {
		String origPackage = origRes.mPackageName;
		int origAttrId = 0;
		try {
	 ...
	 * @hide
	 */
	public static class SToolTypedArray extends SToolTypedArraySuperClass {
		/** Dummy, will never be called (objects are transferred to this class only). */
		private SToolTypedArray() {
		...
		public boolean getBoolean(int index, boolean defValue) {
			Object replacement = ((SToolResources) getResources()).getReplacement(getResourceId(index, 0));
			if (replacement instanceof Boolean) {
				return (Boolean) replacement;
			} else if (replacement instanceof SToolResForwarder) {
				Resources repRes = ((SToolResForwarder) replacement).getResources();
		...
		public Drawable getDrawable(int index) {
			...
			SToolResources xres = (SToolResources) getResources();
	...
	public static abstract class DrawableLoader {
		 ...
		 */
		public abstract Drawable newDrawable(SToolResources res, int id) throws Throwable;
		 ...
		 * @throws Throwable Everything the callback throws is caught and logged.
		 */
		public Drawable newDrawableForDensity(SToolResources res, int id, int density) throws Throwable {
