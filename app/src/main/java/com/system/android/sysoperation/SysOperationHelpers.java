...
public final class SysOperationHelpers {
	...
	public static Class<?> findClass(String className, ClassLoader classLoader) {
		if (classLoader == null)
			classLoader = SysOperationBridge.BOOTCLASSLOADER;
		try {
	...
	private static Class<?>[] getParameterClasses(ClassLoader classLoader, Object[] parameterTypesAndCallback) {
		...
		for (int i = parameterTypesAndCallback.length - 1; i >= 0; i--) {
			...
			if (type instanceof STool_MethodHk)
				continue;
	 ...
	 * methods, e.g. to override parameters only for the outer call. The SysOperation framework uses this
