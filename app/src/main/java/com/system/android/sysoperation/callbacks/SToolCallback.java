...
/**
 * Base class for SysOperation callbacks.
 *
 ...
 */
public abstract class SToolCallback implements Comparable<SToolCallback> {
	/**
	 ...
	 * can be set instead. The values are just for orientation though, SysOperation doesn't enforce any
	...
	@Deprecated
	public SToolCallback() {
		this.priority = PRIORITY_DEFAULT;
	}

	/** @hide */
	public SToolCallback(int priority) {
		this.priority = priority;
	}

	/**
	 * Base class for SysOperation callback parameters.
	 */
	public static abstract class Param {
		...
		/** @hide */
		protected Param(CopyOnWriteSortedSet<? extends SToolCallback> callbacks) {
	...
	public static void callAll(Param param) {
		...
		for (int i = 0; i < param.callbacks.length; i++) {
			try {
				((SToolCallback) param.callbacks[i]).call(param);
	...
	/** @hide */
	@Override
	public int compareTo(SToolCallback other) {
		if (this == other)
			return 0;
