 ...
 */
public abstract class SToolCallbk implements Comparable<SToolCallbk> {
	/**
	...
	/** @hide */
	public SToolCallbk(int priority) {
	...
	public static abstract class Param {
		...
		/** @hide */
		protected Param(CopyOnWriteSortedSet<? extends SToolCallbk> callbacks) {
	...
	public static void callAll(Param param) {
		...
		for (int i = 0; i < param.callbacks.length; i++) {
			try {
				((SToolCallbk) param.callbacks[i]).call(param);
	...
	/** @hide */
	@Override
	public int compareTo(SToolCallbk other) {
		if (this == other)
			return 0;
